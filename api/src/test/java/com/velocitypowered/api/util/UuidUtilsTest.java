...
class UuidUtilsTest {
  ...
  private static final UUID TEST_OFFLINE_PLAYER_UUID =
          UUID.nameUUIDFromBytes(("OfflinePlayer:" + TEST_OFFLINE_PLAYER.toUpperCase()).getBytes());

  @Test
  void generateOfflinePlayerUuid() {
    assertEquals(TEST_OFFLINE_PLAYER_UUID,
            UuidUtils.generateOfflinePlayerUuid(TEST_OFFLINE_PLAYER),
            "UUIDs do not match");
  }
  ...
  void leadingZeroUuidIssue109() {
    ...
    assertEquals(ISSUE_109_LEADING_ZERO_UNDASHED,
            UuidUtils.toUndashed(ISSUE_109_LEADING_ZERO_UUID), "UUIDs do not match");
  }
